(()=>{
  ...
  if('IntersectionObserver' in window&&sections.length){
    ...
    const observer=new IntersectionObserver(entries=>{
      ...
      const visible=sections
        ...
        .sort((a,b)=>Math.abs(a.boundingClientRect.top-125)-Math.abs(b.boundingClientRect.top-125));
      if(visible[0])setCurrent(visible[0].target.id);
    },{rootMargin:'-110px 0px -58% 0px',threshold:[0,.05,.2]});
  ...
  if(!reduced&&'IntersectionObserver' in window){
    const targets=[...document.querySelectorAll('.kyotsu-official-ledger,.kyotsu-regular-grid,.course-price,.kyotsu-fee-examples,.kyotsu-support-grid')];
    const observer=new IntersectionObserver(entries=>{
